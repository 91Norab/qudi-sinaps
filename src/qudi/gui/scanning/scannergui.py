...
class ScannerGui(GuiBase):
    ...
    def on_activate(self):
        ...
        self._mw.action_utility_full_range.triggered.connect(self._scanning_logic().set_full_scan_ranges, QtCore.Qt.QueuedConnection)
    ...
    def _init_scanner_settings(self):
        ...
        self._ssd = ScannerSettingDialog(tuple(self._scanning_logic().scanner_axes.values()), self._scanning_logic().scanner_constraints)
    ...
    def _init_static_dockwidgets(self):
        self.scanner_control_dockwidget = AxesControlDockWidget(tuple(self._scanning_logic().scanner_axes.values()))
        ...
        self._mw.action_view_line_scan.triggered[bool].connect(lambda is_vis: [wid.setVisible(is_vis) for wid in self.scan_1d_dockwidgets.values()])
        self.scanner_control_dockwidget.sigResolutionChanged.connect(lambda ax, res: self.sigScanSettingsChanged.emit({'resolution': {ax: res}}) if not self._scanner_settings_locked else None)
        self.scanner_control_dockwidget.sigRangeChanged.connect(lambda ax, ranges: self.sigScanSettingsChanged.emit({'range': {ax: ranges}}) if not self._scanner_settings_locked else None)
        
        # TODO: When "current target" value box is clicked in, a move is excecuted. Why and how?
        self.scanner_control_dockwidget.sigTargetChanged.connect(lambda ax, pos: self.set_scanner_target_position({ax: pos}))
        
        # TODO: Implement a way to avoid too fast position update from slider movement.
        # TODO: why is _update_scan_crosshairs issuing (not only displaying) at all?
        self.scanner_control_dockwidget.sigSliderMoved.connect(lambda ax, pos: self.set_scanner_target_position({ax: pos}))
            #lambda ax, pos: self._update_scan_markers(pos_dict={ax: pos}, exclude_scan=None)

        self.optimizer_dockwidget = OptimizerDockWidget(axes=self._scanning_logic().scanner_axes, plot_dims=self._optimizer_plot_dims, sequence=self._optimize_logic().scan_sequence)
    ...
    def _add_scan_dockwidget(self, axes):
        ...
        if len(axes) == 1:
            ...
            dockwidget = ScanDockWidget(axes=axes_constr, channels=channel_constr, xy_region_min_size_percentile=self._min_crosshair_size_fraction)
            dockwidget.scan_widget.set_marker_size(marker_size)
            dockwidget.scan_widget.set_marker_bounds(marker_bounds)
            dockwidget.scan_widget.set_plot_range(x_range=axes_constr[0].value_range, y_range=axes_constr[1].value_range)
    ...
    def scanner_settings_updated(self, settings=None):
        ...
        if 'frequency' in settings:
            ...
            new_freq = {ax: (forward, old_freq[ax][1]) for ax, forward in settings['frequency'].items()}
        ...
        if scan_data is not None:
            if caller_id is self._optimizer_id:
                ...
                if scan_data.scan_dimension == 2:
                    ...
                    self.optimizer_dockwidget.set_image(image=scan_data.data[channel], extent=scan_data.scan_range, axs=scan_data.scan_axes)
                    self.optimizer_dockwidget.set_image_label(axis='bottom', text=x_ax, units=scan_data.axes_units[x_ax], axs=scan_data.scan_axes)
                    self.optimizer_dockwidget.set_image_label(axis='left', text=y_ax, units=scan_data.axes_units[y_ax], axs=scan_data.scan_axes)
                elif scan_data.scan_dimension == 1:
                    x_ax = scan_data.scan_axes[0]
                    self.optimizer_dockwidget.set_plot_data(
                        x=np.linspace(*scan_data.scan_range[0], scan_data.scan_resolution[0]), y=scan_data.data[channel], axs=scan_data.scan_axes)
                    self.optimizer_dockwidget.set_plot_label(axis='bottom', text=x_ax, units=scan_data.axes_units[x_ax], axs=scan_data.scan_axes)
                    self.optimizer_dockwidget.set_plot_label(axis='left', text=channel, units=scan_data.channel_units[channel], axs=scan_data.scan_axes)
            else:
                if scan_data.scan_dimension == 2:
    ...
    def update_optimizer_settings(self, settings=None):
        ...
        if 'scan_sequence' in settings:
            ...
            for seq_step in settings['scan_sequence']:
                if len(seq_step) == 1:
                    axis = seq_step[0]
                    self.optimizer_dockwidget.set_plot_label(axis='bottom', axs=seq_step, text=axis, units=axes_constr[axis].unit)
                    ...
                    self.optimizer_dockwidget.set_image_label(axis='bottom', axs=seq_step, text=x_axis, units=axes_constr[x_axis].unit)
                    self.optimizer_dockwidget.set_image_label(axis='left', axs=seq_step, text=y_axis, units=axes_constr[y_axis].unit)
                ...
                if 'data_channel' in settings and len(seq_step)==1:
                    ...
                    self.optimizer_dockwidget.set_plot_label(axs=seq_step, axis='left', text=channel, units=channel_constr[channel].unit)
