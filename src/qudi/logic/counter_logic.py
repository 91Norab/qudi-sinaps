...
class CounterLogic(LogicBase):
    ...
    def set_counting_samples(self, samples=1):
        ...
        The counter is stopped first and restarted afterward.
    ...
    def set_count_length(self, length=300):
        ...
        This makes sure, the counter is stopped first and restarted afterward.
        """
        if self.module_state() == 'locked':
            restart = True
        else:
    ...
    def get_saving_state(self):
        """ Returns if the data is saved at the moment.
    ...
    def save_data(self, to_file=True, postfix='', save_figure=True):
        ...
        metadata = {
            'Start counting time': time.strftime('%d.%m.%Y %Hh:%Mmin:%Ss', time.localtime(self._saving_start_time)),
            'Stop counting time': time.strftime('%d.%m.%Y %Hh:%Mmin:%Ss', time.localtime(self._saving_stop_time)),
            'Count frequency (Hz)':self._count_frequency,
            'Oversampling (Samples)':self._counting_samples,
        ...
        if to_file:
            ...
            filepath, _, _ = storage.save_data(self._data_to_save, metadata=metadata)
            if fig is not None:
        ...
        self.sigSavingStatusChanged.emit(self._saving)
        return self._data_to_save, metadata
    ...
    def startCount(self):
        """ This is called externally, and is basically a wrapper that redirects to
        the chosen counting mode start function.
        ...
        if self._counting_mode not in constraints.counting_mode:
            self.log.error('Unknown counting mode "{0}". Cannot start the counter.'.format(self._counting_mode))
    ...
    def save_current_count_trace(self, name_tag='', save_figure=True):
        ...
        The counttrace variable will be saved to file with the provided name!
        """
        ...
        metadata =  {'Saved at time': timestr,
            'Count frequency (Hz)': self._count_frequency,
            'Oversampling (Samples)': self._counting_samples,
            'Smooth Window Length (# of events)': self._smooth_window_length}
        ...
        storage = TextDataStorage(root_dir=self.module_default_data_dir)
        filepath, _, _ = storage.save_data(data, metadata=metadata)
        
        if save_figure:
            fig = self.draw_figure(data=np.array(data))
        else:
            fig = None
        ...
        return data, filepath, metadata, filelabel
    ...
    def _stopCount_wait(self, timeout=5.0):
        ...
        @param timeout: float, the max. time in seconds how long the method should wait for the process to stop.
        @return: error code
        """
        self.stopCount()
